...
# Portable builds share the same source tree, so they need this too.
...
Write-Host "[1/4] Building Tauri portable (demo,portable features)..." -ForegroundColor Yellow
npx tauri build --features demo,portable --config src-tauri/tauri.conf.portable.json
if ($LASTEXITCODE -ne 0) {
    ...
    $expected = "src-tauri\target\release\bundle\nsis\Datapilot Scout Portable_${version}_x64-setup.exe"
...
# Find the NSIS installer for THIS version (portable variant)
$installer = Get-ChildItem "src-tauri\target\release\bundle\nsis\Datapilot Scout Portable_${version}_x64-setup.exe" -ErrorAction SilentlyContinue | Select-Object -First 1
if (-Not $installer) {
    Write-Host "ERROR: Portable NSIS installer not found for version $version" -ForegroundColor Red
    Write-Host "  Expected: src-tauri\target\release\bundle\nsis\Datapilot Scout Portable_${version}_x64-setup.exe" -ForegroundColor Red
    exit 1
}
...
Write-Host "  Product: Portable USB" -ForegroundColor White
